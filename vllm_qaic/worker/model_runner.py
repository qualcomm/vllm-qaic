...
class QaicModelRunnerAoT(GPUModelRunner):
    ...
    def _prepare_qaic_inputs(
        ...
        sampling_generators: dict[int, torch.Generator] = {}
        self.input_batch.sampling_metadata.generators = sampling_generators
        for request_index, request_id in enumerate(self.input_batch.req_ids):
            generator = self.requests[request_id].generator
            if generator is None:
                continue
            if self.discard_request_mask.np[request_index]:
                sampling_generators[request_index] = generator.clone_state()
                self.input_batch.generators.pop(request_index, None)
                continue
            sampling_generators[request_index] = generator
            self.input_batch.generators[request_index] = generator
    ...
    def execute_model(
        ...
        if self.is_pooling_model and not self.model_config.is_multimodal_model:
            if not self.use_async_scheduling:
                assert isinstance(pending_prefill_exec_queue, torch.Tensor)
                return self._pool(
                ...
                assert isinstance(pending_prefill_exec_queue, Queue)
