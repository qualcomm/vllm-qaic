"""
Standalone QAIC validation for `fused_gdn_gating_kernel`.

Source under test:
vllm/model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py
  - fused_gdn_gating_kernel  (Fused kernel computing the softplus-based
    decay gate `g` and sigmoid beta-output gate for Qwen GDN attention.)
...
LOG_FILE = os.path.join(LOG_DIR, "log_fused_gdn_gating_kernel.txt")
KERNEL_FILE_PATH = "vllm/model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py"
...
def pytorch_ref(A_log, a, b, dt_bias, beta, threshold):
...
def kernel_impl(A_log, a, b, dt_bias, beta, threshold):
    ...
    Requirements (Claude.md):
      ...
      - No validation logic.
    """
    return fused_gdn_gating(A_log, a, b, dt_bias, beta=beta, threshold=threshold)
...
def main():
    ...
    try:
        ref_g, ref_beta = pytorch_ref(A_LOG, A_INPUT, B_INPUT, DT_BIAS, BETA, THRESHOLD)
        kernel_g, kernel_beta = kernel_impl(
            A_LOG, A_INPUT, B_INPUT, DT_BIAS, BETA, THRESHOLD
        )

        ref_g_cpu = ref_g.cpu()
        ref_beta_cpu = ref_beta.cpu()
        kernel_g_cpu = kernel_g.cpu()
        kernel_beta_cpu = kernel_beta.cpu().float()
        ...
        rel_err_g = (diff_g.max() / (ref_g_cpu.abs().max() + 1e-8)).item()
        rel_err_beta = (diff_beta.max() / (ref_beta_cpu.abs().max() + 1e-8)).item()
        ...
        pt_stats = _bench(
            lambda: pytorch_ref(A_LOG, A_INPUT, B_INPUT, DT_BIAS, BETA, THRESHOLD)
        )
        kern_stats = _bench(
            lambda: kernel_impl(A_LOG, A_INPUT, B_INPUT, DT_BIAS, BETA, THRESHOLD)
        )
        speedup = (
