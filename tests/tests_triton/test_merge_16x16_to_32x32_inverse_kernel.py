"""
Standalone QAIC validation for `merge_16x16_to_32x32_inverse_kernel`.

Source under test:
vllm/model_executor/layers/fla/ops/solve_tril.py
  - merge_16x16_to_32x32_inverse_kernel  (Merges four 16x16 triangular-inverse
    blocks into a 32x32 block-triangular matrix inverse.)

Launcher `solve_tril(A, cu_seqlens=None, chunk_indices=None, output_dtype)`
dispatches to this exact kernel when A.shape[-1] == 32.
...
LOG_FILE = os.path.join(LOG_DIR, "log_merge_16x16_to_32x32_inverse_kernel.txt")
...
# merge_16x16_to_32x32_inverse_kernel is wrapped in @triton.autotune. Triton's
...
def pytorch_ref(A):
    """Pure PyTorch reference implementation for
...
def main():
    ...
    try:
        ...
        lines = [
            f"{timestamp}\n",
            "Kernel: merge_16x16_to_32x32_inverse_kernel\n",
