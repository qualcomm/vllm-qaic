"""
Standalone QAIC validation for `layer_norm_gated_fwd_kernel`.

Source under test:
vllm/model_executor/layers/fla/ops/kda.py
  - layer_norm_gated_fwd_kernel  (@triton.jit, block-of-rows variant)
  - layer_norm_gated_fwd         (launcher; D <= 512 dispatches to this kernel)

Gated (RMS/layer) norm with an output gate. For the config exercised here
(is_rms_norm=True, weight given, bias=None, residual=None, activation="swish"):
    rstd  = 1 / sqrt(mean(x^2, dim=-1) + eps)
    x_hat = x * rstd
    y     = x_hat * weight
    y     = y * g * sigmoid(g)          # swish/silu output gate
This kernel (the non-"1" variant) processes BT rows per program via block
pointers and is selected by the launcher when D <= 512. We pass out_dtype so
the launcher writes a fresh output tensor (default would overwrite x in place).
...
LOG_FILE = os.path.join(LOG_DIR, "log_layer_norm_gated_fwd_kernel.txt")
...
# D <= 512 so the launcher selects layer_norm_gated_fwd_kernel (BT=32).
T = 64
D = 256
EPS = 1e-5
ACTIVATION = "swish"
...
def main():
    ...
    try:
        ...
        lines = [
            f"{timestamp}\n",
            "Kernel: layer_norm_gated_fwd_kernel (D<=512 variant)\n",
