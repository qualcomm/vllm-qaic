"""
Standalone QAIC validation for `_e2m1_lookup`.

Source under test:
vllm/model_executor/layers/quantization/utils/nvfp4_emulation_utils.py
  - _e2m1_lookup  (@triton.jit DEVICE HELPER)

`_e2m1_lookup(magnitude)` maps a 3-bit E2M1 magnitude code (0..7) to its float
value via a chain of tl.where comparisons, producing the same grid as
`_e2m1_inline`: {0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0}. VERIFIED against the
source: code 0 -> 0.0 (the initial default), then 1->0.5, 2->1.0, 3->1.5,
4->2.0, 5->3.0, 6->4.0, 7->6.0. This matches test_e2m1_inline.py's grid exactly.

Device helper -> wrapped in a tiny standalone @triton.jit kernel that loads the
codes, calls the helper, and stores the float results.

Reference: pure-PyTorch E2M1 magnitude lookup. FLOAT/exact compare at
rtol/atol=1e-3.
"""
...
LOG_FILE = os.path.join(LOG_DIR, "log_e2m1_lookup.txt")
KERNEL_FILE_PATH = (
    "vllm/model_executor/layers/quantization/utils/nvfp4_emulation_utils.py"
)
...
@triton.jit
def _e2m1_lookup_wrapper(code_ptr, out_ptr, N: tl.constexpr):
    offs = tl.arange(0, N)
    mag = tl.load(code_ptr + offs)
    val = _e2m1_lookup(mag)
...
def kernel_impl(codes):
    ...
    _e2m1_lookup_wrapper[(1,)](codes, out, N)
    return out
...
def main():
    ...
    try:
        ...
        lines = [
            f"{timestamp}\n",
            "Kernel: _e2m1_lookup (device helper, wrapped)\n",
