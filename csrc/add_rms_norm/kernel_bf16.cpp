...
//   [5]=epsilon (float scalar), [6]=B (int scalar), [7]=M (int scalar), [8]=N
//   (int scalar)
// Each core owns a stripe of rows across all B*M rows.
// Double-buffered DMA (slots 0/1) overlaps prefetch of row r+1 with compute on
// row r.
QAIC_KERNEL_API uint32_t rms_norm_multi_nsp_bf16(
  ...
  const uint32_t requiredVtcmBytes = 5 * rowBytesAligned +
                                     2 * rowBytesF32Aligned + partialBytes +
                                     statusBytes + handleBytes + kAlign;
  ...
  for (int iter = 0; iter < rowIters; ++iter) {
    ...
    if (validRow) {
      _single_nsp_rms_norm_bf16(attn_vtcm[cur_slot], x_vtcm[cur_slot],
                                weight_vtcm, out_vtcm, dst_vtcm, epsilon, N,
                                threadID, localThreadID, numThreads,
                                partial_sums);
    } else {
      // Idle row: _single_nsp_rms_norm_bf16 contains 3 sync barriers; mirror
      // them to keep all threads in lockstep
      sync_hvx_threads(threadID, numThreads);
      sync_hvx_threads(threadID, numThreads);
      sync_hvx_threads(threadID, numThreads);
    }
    ...
    if (localThreadID == 0) {
      ...
      if (validRow && *status_vtcm == JIT_DEV_STATUS_SUCCESS) {
        *status_vtcm =
            dma_copy_wait(threadID, dst_ddr + r * N, dst_vtcm, rowBytesF32);
      }
      if (validRow && *status_vtcm == JIT_DEV_STATUS_SUCCESS) {
        *status_vtcm =
            dma_copy_wait(threadID, out_ddr + r * N, out_vtcm, rowBytesF32);
      }
    }
