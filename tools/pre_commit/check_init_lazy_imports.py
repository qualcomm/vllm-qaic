...
# ------------------------------------------------------------------
#
# Adapted from vllm/tools/pre_commit/check_init_lazy_imports.py
"""Ensure we perform lazy loading in vllm_qaic/__init__.py.
i.e: internal (``vllm_qaic.*``) imports appear only within the
``if typing.TYPE_CHECKING:`` guard, **except** for a short whitelist.
"""
...
INIT_PATH: Final = Path("vllm_qaic/__init__.py")

# If you need to add items to whitelist, do it here.
ALLOWED_IMPORTS: Final[frozenset[str]] = frozenset(set())
...
def _is_internal(name: str | None, *, level: int = 0) -> bool:
    ...
    if name is None:
        return False
    return name.startswith("vllm_qaic.") or name == "vllm_qaic"


def _fail(violations: Iterable[tuple[int, str]]) -> None:
    print(
        "ERROR: Disallowed eager imports in vllm_qaic/__init__.py:\n",
        file=sys.stderr,
    )
...
def main() -> None:
    ...
    class Visitor(ast.NodeVisitor):
        ...
        def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
            # Imports nested inside functions are already lazy (only run
        ...
        def visit_ClassDef(self, node: ast.ClassDef) -> None:
            # Method bodies are handled by visit_FunctionDef; class-body
            # imports are rare and, if present, should be lazy too.
            return
