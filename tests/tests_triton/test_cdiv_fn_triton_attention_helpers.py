...
`@triton.jit` DEVICE HELPER shared by the unified attention kernel and
`reduce_segments` to size tile loops, so we wrap it in a tiny standalone
`@triton.jit` kernel that evaluates it elementwise over a sweep of (a, b).

This is a distinct source location from the chunked-prefill/paged-decode copy;
...
from vllm.v1.attention.ops.triton_attention_helpers import cdiv_fn

LOG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "logs_v23")
LOG_FILE = os.path.join(LOG_DIR, "log_cdiv_fn_triton_attention_helpers.txt")
