...
static inline HVX_VectorPair bf16_to_sf_pair(HVX_Vector v) {
    HVX_Vector zero = Q6_V_vzero();
    HVX_VectorPair interleaved = Q6_W_vshuff_VVR(v, zero, -2);
    HVX_Vector lo = Q6_Vw_vasl_VwR(Q6_V_lo_W(interleaved), 16);
    HVX_Vector hi = Q6_Vw_vasl_VwR(Q6_V_hi_W(interleaved), 16);
    return Q6_W_vcombine_VV(hi, lo);
}

static inline HVX_VectorPair vadd_bf16_as_sf(HVX_Vector a, HVX_Vector b) {
    HVX_VectorPair a_sf = bf16_to_sf_pair(a);
    HVX_VectorPair b_sf = bf16_to_sf_pair(b);
    HVX_Vector lo = Q6_Vsf_vadd_VsfVsf(Q6_V_lo_W(a_sf), Q6_V_lo_W(b_sf));
    HVX_Vector hi = Q6_Vsf_vadd_VsfVsf(Q6_V_hi_W(a_sf), Q6_V_hi_W(b_sf));
    return Q6_W_vcombine_VV(hi, lo);
}
...
static QShimUDmaHandle qaic_linear_udma_submit(
    uint32_t threadId,
    AicJitPtr src,
    uint32_t size,
    AicJitPtr dst,
    ...
    return qshimLinearUdmaSubmit(threadId, src, size, dst, &udmaDescAttrs, requireHandle, status);
}

static inline uint32_t dma_copy_wait(
    uint32_t threadId,
    void* dst,
    const void* src,
    uint32_t bytes,
...
static inline QShimUDmaHandle dma_copy_submit(
    uint32_t threadId,
    void* dst,
    const void* src,
    uint32_t bytes,
    ...
    return qaic_linear_udma_submit(
        threadId,
        (AicJitPtr)src,
        bytes,
        (AicJitPtr)dst,
...
extern "C" void _single_nsp_rms_norm_bf16(
    ...
    float* out,          // residual output (attn_out + x), written in phase 1 as fp32
    float* dst,          // normalized output, written in phase 2 as fp32
    float epsilon,
    int N,
    uint32_t threadID,
    ...
    for (int i = (int)localThreadID; i < vlen; i += (int)threadsPerCore) {
        ...
        // output is fp32: each bf16 input vector expands to two fp32 vectors
        float* tmp_lo = out + i * elems_per_vec * 2;
        float* tmp_hi = out + i * elems_per_vec * 2 + (elems_per_vec / 2) * 2;

        HVX_Vector vec1 = *(HVX_Vector*)src1_ptr;
        HVX_Vector vec2 = *(HVX_Vector*)src2_ptr;

        HVX_VectorPair sum_f32 = vadd_bf16_as_sf(vec1, vec2);

        // Store as fp32 (no bf16 conversion)
        *(HVX_Vector*)tmp_lo = Q6_V_lo_W(sum_f32);
        *(HVX_Vector*)tmp_hi = Q6_V_hi_W(sum_f32);

        HVX_Vector sq_lo = Q6_Vsf_vmpy_VsfVsf(Q6_V_lo_W(sum_f32), Q6_V_lo_W(sum_f32));
        HVX_Vector sq_hi = Q6_Vsf_vmpy_VsfVsf(Q6_V_hi_W(sum_f32), Q6_V_hi_W(sum_f32));
    ...
    // Cross-thread reduction: each thread writes its partial sum, thread 0 computes inv_rms
    ...
    if (localThreadID == 0) {
        ...
        float inv_rms = 1.0f / __builtin_sqrtf((total_sum_f32 / (float)N) + epsilon);
    ...
    for (int i = (int)localThreadID; i < vlen; i += (int)threadsPerCore) {
        float* residual_lo = out + i * elems_per_vec * 2;
        float* residual_hi = out + i * elems_per_vec * 2 + (elems_per_vec / 2) * 2;
        const uint16_t* weight_ptr = weight + i * elems_per_vec;
        float* dst_lo = dst + i * elems_per_vec * 2;
        float* dst_hi = dst + i * elems_per_vec * 2 + (elems_per_vec / 2) * 2;

        HVX_Vector res_lo_f32 = *(HVX_Vector*)residual_lo;
        HVX_Vector res_hi_f32 = *(HVX_Vector*)residual_hi;
        HVX_Vector w_v = *(HVX_Vector*)weight_ptr;

        HVX_VectorPair w_f32 = bf16_to_sf_pair(w_v);

        HVX_Vector norm_lo = Q6_Vsf_vmpy_VsfVsf(res_lo_f32, inv_rms_sf);
        HVX_Vector norm_hi = Q6_Vsf_vmpy_VsfVsf(res_hi_f32, inv_rms_sf);

        HVX_Vector out_lo = Q6_Vsf_vmpy_VsfVsf(norm_lo, Q6_V_lo_W(w_f32));
        HVX_Vector out_hi = Q6_Vsf_vmpy_VsfVsf(norm_hi, Q6_V_hi_W(w_f32));

        // Store as fp32 — no Q6_Vbf_vcvt_VsfVsf needed
        *(HVX_Vector*)dst_lo = out_lo;
        *(HVX_Vector*)dst_hi = out_hi;
    }
...
QAIC_KERNEL_API uint32_t rms_norm_multi_nsp_bf16(
    const AicJitEntryPointConfig* cfg,
...
)
{
    ...
    // N must be an exact multiple of the HVX vector width (128 bytes / 2 bytes = 64 elements)
    if ((N % elems_per_vec) != 0) {
        return JIT_DEV_ERROR_INVALID_PARAMETER;
    }

    // VTCM layout (all 128-byte aligned):
    //   attn_vtcm[0/1]  — double-buffer for attn_out rows (bf16)
    //   x_vtcm[0/1]     — double-buffer for x rows (bf16)
    //   weight_vtcm     — weight row (bf16, loaded once, reused for every row)
    //   out_vtcm        — residual scratch (fp32 output, 2x rowBytes)
    //   dst_vtcm        — normalized output scratch (fp32 output, 2x rowBytes)
    ...
    const uint32_t rowBytesF32 = (uint32_t)N * sizeof(float);
    const uint32_t rowBytesAligned = align_up_u32(rowBytes, kAlign);
    const uint32_t rowBytesF32Aligned = align_up_u32(rowBytesF32, kAlign);
    const uint32_t partialBytes = align_up_u32(numThreads * sizeof(float), kAlign);
    ...
    const uint32_t requiredVtcmBytes = 5 * rowBytesAligned + 2 * rowBytesF32Aligned
        + partialBytes + statusBytes + handleBytes + kAlign;
    ...
    vtcmPtr += rowBytesF32Aligned;
    float* dst_vtcm = (float*)vtcmPtr;
    vtcmPtr += rowBytesF32Aligned;
    ...
    QShimUDmaHandle* prefetch_handle_vtcm = (QShimUDmaHandle*)align_up_ptr(vtcmPtr, kAlign);
    ...
    // Prime slot 0 with the first row assigned to this core before entering the loop
    {
    ...
    for (int iter = 0; iter < rowIters; ++iter) {
        ...
        // Thread 0 submits async DMA for the next row while all threads compute the current row.
        // attn prefetch is waited immediately (sequential); x prefetch is left in-flight
        // and waited after compute completes to maximally overlap with rms_norm_bf16 execution.
        if (localThreadID == 0) {
            if (validNextRow) {
                uint32_t dma_status = JIT_DEV_STATUS_SUCCESS;
                QShimUDmaHandle h_attn = dma_copy_submit(
                    threadID, attn_vtcm[next_slot],
                ...
                if (dma_status != JIT_DEV_STATUS_SUCCESS ||
                    ...
                    *status_vtcm = (dma_status != JIT_DEV_STATUS_SUCCESS)
                        ? dma_status : JIT_DEV_ERROR_INVALID_PARAMETER;
                    *prefetch_handle_vtcm = INVALID_UDMA_HANDLE;
                } else {
                    ...
                    if (wait_st != JIT_DEV_STATUS_SUCCESS) {
                        ...
                        *prefetch_handle_vtcm = INVALID_UDMA_HANDLE;
                        ...
                        // Kick off x DMA and leave it in-flight; save handle for post-compute wait
                        QShimUDmaHandle h_x = dma_copy_submit(
                            threadID, x_vtcm[next_slot],
                            x_ddr + r_next * N, rowBytes, &dma_status);
                        *prefetch_handle_vtcm =
                            (dma_status == JIT_DEV_STATUS_SUCCESS) ? h_x : INVALID_UDMA_HANDLE;
                        if (dma_status != JIT_DEV_STATUS_SUCCESS) {
        ...
        if (validRow) {
            _single_nsp_rms_norm_bf16(
                attn_vtcm[cur_slot], x_vtcm[cur_slot], weight_vtcm,
                out_vtcm, dst_vtcm, epsilon, N,
                threadID, localThreadID, numThreads, partial_sums);
        } else {
            // Idle row: _single_nsp_rms_norm_bf16 contains 3 sync barriers; mirror them to keep all threads in lockstep
            sync_hvx_threads(threadID, numThreads);
            sync_hvx_threads(threadID, numThreads);
            sync_hvx_threads(threadID, numThreads);
        }

        // After compute: wait for the in-flight x prefetch, then DMA the result back to DDR
        if (localThreadID == 0) {
            ...
            if (validRow && *status_vtcm == JIT_DEV_STATUS_SUCCESS) {
                *status_vtcm = dma_copy_wait(
                    threadID, dst_ddr + r * N, dst_vtcm, rowBytesF32);
            }
            if (validRow && *status_vtcm == JIT_DEV_STATUS_SUCCESS) {
                *status_vtcm = dma_copy_wait(
                    threadID, out_ddr + r * N, out_vtcm, rowBytesF32);
            }
        }
