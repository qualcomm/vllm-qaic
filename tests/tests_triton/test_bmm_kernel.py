"""
Standalone QAIC validation for `bmm_kernel`.

Source under test:
vllm/model_executor/layers/batch_invariant.py
  - bmm_kernel  (deterministic batched GEMM, batch-invariant torch.bmm)

Computes C[b] = A[b] @ B[b] for each batch element with a fixed K-reduction
order, the deterministic (batch-invariant) replacement for torch.bmm.
Launched through the file's `bmm_batch_invariant` wrapper.

Reference: pure PyTorch torch.bmm(A, B). Float32, rtol/atol=1e-3.
"""
...
LOG_FILE = os.path.join(LOG_DIR, "log_bmm_kernel.txt")
