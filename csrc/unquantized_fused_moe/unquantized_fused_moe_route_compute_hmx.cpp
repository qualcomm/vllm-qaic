...
namespace unquantized_fused_moe_route_compute_hmx {
...
#define QAIC_MOE_SHUFFLE_EVEN_HF                                                       \
  0, 1, 4, 5, 8, 9, 12, 13, 16, 17, 20, 21, 24, 25, 28, 29, 32, 33, 36, 37, 40, 41,   \
      44, 45, 48, 49, 52, 53, 56, 57, 60, 61, 64, 65, 68, 69, 72, 73, 76, 77, 80, 81, \
      84, 85, 88, 89, 92, 93, 96, 97, 100, 101, 104, 105, 108, 109, 112, 113, 116,    \
      117, 120, 121, 124, 125, 128, 129, 132, 133, 136, 137, 140, 141, 144, 145,      \
      148, 149, 152, 153, 156, 157, 160, 161, 164, 165, 168, 169, 172, 173, 176,      \
      177, 180, 181, 184, 185, 188, 189, 192, 193, 196, 197, 200, 201, 204, 205,      \
      208, 209, 212, 213, 216, 217, 220, 221, 224, 225, 228, 229, 232, 233, 236,      \
      237, 240, 241, 244, 245, 248, 249, 252, 253

#define QAIC_MOE_SHUFFLE_ODD_HF                                                        \
  2, 3, 6, 7, 10, 11, 14, 15, 18, 19, 22, 23, 26, 27, 30, 31, 34, 35, 38, 39, 42,     \
      43, 46, 47, 50, 51, 54, 55, 58, 59, 62, 63, 66, 67, 70, 71, 74, 75, 78, 79,     \
      82, 83, 86, 87, 90, 91, 94, 95, 98, 99, 102, 103, 106, 107, 110, 111, 114,      \
      115, 118, 119, 122, 123, 126, 127, 130, 131, 134, 135, 138, 139, 142, 143,      \
      146, 147, 150, 151, 154, 155, 158, 159, 162, 163, 166, 167, 170, 171, 174,      \
      175, 178, 179, 182, 183, 186, 187, 190, 191, 194, 195, 198, 199, 202, 203,      \
      206, 207, 210, 211, 214, 215, 218, 219, 222, 223, 226, 227, 230, 231, 234,      \
      235, 238, 239, 242, 243, 246, 247, 250, 251, 254, 255
...
inline float16* alloc_aligned_hf(uint8_t* base,
                                 uint64_t* offset,
                                 uint32_t alignment,
                                 uint32_t bytes) {
...
inline bool init_hmx_scratch(uint8_t* vtcm_base_raw,
                             uint32_t vtcm_size,
  ...
  for (uint32_t row_i = 0; row_i < sizeof(kRowCandidates) / sizeof(kRowCandidates[0]);
       ++row_i) {
    ...
    for (uint32_t col_i = 0;
         col_i < sizeof(kColCandidates) / sizeof(kColCandidates[0]);
         ++col_i) {
      ...
      if (required + base_slop <= vtcm_size) {
        ...
        offset += kMaxKernelThreads * sizeof(uint32_t);
        scratch->lhs_rm = alloc_aligned_hf(
            vtcm_base,
            &offset,
            QAIC_HMX_INPUT_ALIGNMENT,
            (uint32_t)((uint64_t)route_rows * max_common_dim * sizeof(float16)));
        scratch->rhs_cm = alloc_aligned_hf(
            vtcm_base,
            &offset,
            QAIC_HMX_INPUT_ALIGNMENT,
            (uint32_t)((uint64_t)column_cols * max_common_dim * sizeof(float16)));
        scratch->lhs_crouton = alloc_aligned_hf(
            vtcm_base,
            &offset,
            QAIC_HMX_ALIGNMENT,
        ...
        scratch->hidden_batch = alloc_aligned_hf(
            vtcm_base,
            &offset,
            QAIC_HMX_INPUT_ALIGNMENT,
            (uint32_t)((uint64_t)route_rows * intermediate_size * sizeof(float16)));
...
inline void copy_linear_hf_hvx(const float16* src,
                               float16* dst,
                               int32_t elements,
                               uint32_t hvx_thread_id,
                               uint32_t hvx_threads) {
  if (hvx_thread_id >= hvx_threads) {
    return;
  }
  constexpr int32_t kElemsPerHalfVector = sizeof(HVX_Vector) / sizeof(float16);
  const int32_t vector_elems = (elements / kElemsPerHalfVector) * kElemsPerHalfVector;
...
inline void gather_route_lhs_hvx(const float16* x,
                                 ...
                                 int32_t route_offset,
                                 int32_t actual_routes,
                                 int32_t topk,
                                 int32_t hidden_size,
                                 float16* lhs_rm,
                                 uint32_t hvx_thread_id,
  ...
  for (int32_t row = (int32_t)hvx_thread_id; row < actual_routes;
    ...
    for (int32_t h = 0; h < vector_elems; h += kElemsPerHalfVector) {
      HVX_Vector value = LoadUnaligned<HVX_Vector>((const int8_t*)(src_row + h));
      StoreUnalignedHVX((int8_t*)(dst_row + h), value);
    }
    if (tail > 0) {
      HVX_Vector value = LoadUnaligned<HVX_Vector>(
          (const int8_t*)(src_row + vector_elems),
          tail * (int32_t)sizeof(float16));
...
inline HVX_Vector postprocess_hf_vector(HVX_Vector value_hf,
                                        HVX_Vector bias_hf,
                                        bool has_bias,
...
inline void store_w13_tile_hvx(const float16* out_rm,
                               float16* gate_up_batch,
  ...
  if (activation_id == kSwigluOAI) {
    ...
    for (int32_t row = (int32_t)hvx_thread_id; row < actual_routes;
      ...
      for (; col + 1 < cols;) {
        ...
        HVX_Vector bias_hf =
            has_bias
                ? LoadUnaligned<HVX_Vector>((const int8_t*)(bias_row + col), bytes)
                : Q6_V_vzero();
        HVX_Vector out_hf = postprocess_hf_vector(
            value_hf,
            bias_hf,
            has_bias,
            route_weight_sf,
        ...
        if ((w13_col & 1) == 0) {
          StoreUnalignedHVX((int8_t*)(dst_row + pair_start),
                            even_hf,
                            store_bytes);
          ...
          StoreUnalignedHVX((int8_t*)(dst_row + intermediate_size + pair_start),
                            even_hf,
                            store_bytes);
          StoreUnalignedHVX((int8_t*)(dst_row + pair_start + 1),
                            odd_hf,
                            store_bytes);
        }
        col += chunk_cols;
  ...
  constexpr int32_t kElemsPerHalfVector = sizeof(HVX_Vector) / sizeof(float16);
  const int32_t vector_elems = (cols / kElemsPerHalfVector) * kElemsPerHalfVector;
  ...
  for (int32_t row = (int32_t)hvx_thread_id; row < actual_routes;
    ...
    for (int32_t col = 0; col < vector_elems; col += kElemsPerHalfVector) {
      HVX_Vector value_hf = LoadUnaligned<HVX_Vector>((const int8_t*)(src_row + col));
      HVX_Vector bias_hf = has_bias
                                ? LoadUnaligned<HVX_Vector>(
                                      (const int8_t*)(bias_row + col))
                                : Q6_V_vzero();
      HVX_Vector out_hf = postprocess_hf_vector(
          value_hf,
          bias_hf,
...
inline void apply_activation_batch_hvx(const float16* gate_up_batch,
                                       float16* hidden_batch,
                                       int32_t actual_routes,
                                       int32_t w13_dim,
                                       int32_t intermediate_size,
...
inline void scatter_w2_tile_hvx(const float16* out_rm,
                                float16* route_out,
...
inline __attribute__((always_inline)) uint32_t run_hmx_matmul_rhs_cw_tile(
    const float16* lhs_rm_src,
    float16* lhs_crouton,
    const float16* rhs_cm_src,
    float16* rhs_cm,
    float16* rhs_crouton,
    float16* out_crouton,
    float16* out_rm,
  ...
  const QAicHMXDims<QAIC_HMX_FlatNXYD> lhs_dims(
  ...
  const QAicHMXDims<QAIC_HMX_FlatNXYD> out_dims(
...
inline __attribute__((always_inline)) uint32_t process_expert_routes_hmx(
    const float16* x,
    const float16* topk_weights,
    const float16* w13_weight,
    const float16* w2_weight,
    const float16* w13_bias,
    const float16* w2_bias,
    float16* route_out,
    const float* expert_route_indices,
  ...
  for (int32_t route_offset = 0; route_offset < route_count;
    ...
    gather_route_lhs_hvx(x,
                         expert_route_indices,
                         route_begin + route_offset,
                         actual_routes,
                         topk,
                         hidden_size,
                         scratch.lhs_rm,
    ...
    for (int32_t col_start = 0; col_start < w13_dim;
      ...
      uint32_t status = run_hmx_matmul_rhs_cw_tile(
          scratch.lhs_rm,
          scratch.lhs_crouton,
          w13_weight + (int64_t)col_start * hidden_size,
          scratch.rhs_cm,
          scratch.rhs_crouton,
          scratch.out_crouton,
          scratch.out_rm,
      ...
      store_w13_tile_hvx(scratch.out_rm,
                         scratch.gate_up_batch,
                         has_bias ? w13_bias : nullptr,
                         topk_weights,
                         expert_route_indices,
                         route_begin + route_offset,
                         actual_routes,
    ...
    apply_activation_batch_hvx(scratch.gate_up_batch,
    ...
    for (int32_t col_start = 0; col_start < hidden_size;
      ...
      uint32_t status = run_hmx_matmul_rhs_cw_tile(
          scratch.hidden_batch,
          scratch.lhs_crouton,
          w2_weight + (int64_t)col_start * intermediate_size,
          scratch.rhs_cm,
          scratch.rhs_crouton,
          scratch.out_crouton,
          scratch.out_rm,
      ...
      scatter_w2_tile_hvx(scratch.out_rm,
                          route_out,
                          has_bias ? w2_bias : nullptr,
                          topk_weights,
                          expert_route_indices,
                          route_begin + route_offset,
                          actual_routes,
...
inline uint32_t route_compute_hmx_kernel_main(const AicJitEntryPointConfig* cfg,
  ...
  HmxScratch scratch = {};
  if (!init_hmx_scratch(qshimGetBaseVtcmAddr(),
                        (uint32_t)vtcm_size_i64,
                        total_routes,
                        hidden_size,
                        w13_dim,
                        intermediate_size,
  ...
  for (int32_t expert = 0; expert < num_experts; ++expert) {
    ...
    status = process_expert_routes_hmx(x,
                                       topk_weights,
                                       expert_w13,
                                       expert_w2,
                                       expert_bias,
...
QAIC_KERNEL_API int32_t multinsp_multithreaded_unquantized_fused_moe_route_compute_hmx(
    const AicJitEntryPointConfig* entryConfig,
    const AicJitPointerArray* pointerArray) {
  return unquantized_fused_moe_route_compute_hmx::route_compute_hmx_kernel_main(
