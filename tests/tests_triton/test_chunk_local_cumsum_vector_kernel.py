"""
Standalone QAIC validation for `chunk_local_cumsum_vector_kernel`.

Source under test:
vllm/model_executor/layers/fla/ops/cumsum.py
  - chunk_local_cumsum_vector_kernel  (chunk-local causal cumulative sum of a
    vector-valued per-head gating/decay signal, reset every `chunk_size`
    tokens, applied independently across the trailing channel dimension S)

For each chunk of `chunk_size` tokens, per (batch, head), the kernel computes
a causal cumsum along the time axis independently for each of the S
channels -- equivalent to applying `chunk_local_cumsum_scalar` elementwise
over S. Implemented in Triton via a matmul with a lower-triangular
(or upper-triangular when reverse) ones mask against the chunk block.

Reference: pure PyTorch per-chunk `torch.cumsum` along dim=1 (T), broadcast
over the trailing S dimension.
"""
...
LOG_FILE = os.path.join(LOG_DIR, "log_chunk_local_cumsum_vector_kernel.txt")
...
# chunk_local_cumsum_vector_kernel is wrapped in @triton.autotune. Triton's
...
G = torch.randn(B, T, H, S, dtype=torch.float32, device=DEVICE)


def pytorch_ref(g, chunk_size, reverse=False):
    """Pure PyTorch reference for chunk-local causal cumulative sum
    applied elementwise over the trailing channel dim S.

    g: [B, T, H, S] (head_first=False). Within each chunk of `chunk_size`
    tokens along dim=1, computes the causal cumsum independently per chunk
    for every channel s in S (resets every chunk_size tokens).
    """
    B_, T_, H_, S_ = g.shape
