"""
Standalone QAIC validation for `_w8a8_block_int8_matmul`.

Source under test:
vllm/model_executor/layers/quantization/utils/int8_utils.py
  - _w8a8_block_int8_matmul  (@triton.jit)
  - w8a8_block_int8_matmul   (launcher)

Block-wise-quantized INT8 (W8A8) GEMM. `A` (M, K) is int8 with per-token-group
scales `As` (M, K/block_k); `B` (N, K) is int8 with per-block scales `Bs`
(N/block_n, K/block_k). The kernel accumulates int8 dot products per K-block
and rescales each tile:
    C[m, n] = sum_kb ( sum_{k in kb} A[m,k] * B[n,k] ) * As[m, kb] * Bs[n//bn, kb]
which equals dequant(A) @ dequant(B).T.

Reference: pure PyTorch block-dequant of A and B then `A_deq @ B_deq.T`. Using
the same int8 codes/scales, the only divergence is fp32 accumulation order, so
compare at rtol/atol=1e-2.
"""
...
LOG_FILE = os.path.join(LOG_DIR, "log_w8a8_block_int8_matmul.txt")
KERNEL_FILE_PATH = (
    "vllm/model_executor/layers/quantization/utils/int8_utils.py"
)
...
_IS_CHILD = os.environ.get("W8A8_INT8_MM_CHILD") == "1"
...
if _IS_CHILD or __name__ != "__main__":
    ...
    # Quantize A with per-token-group (group == block_k) int8 scales.
    A_CODE, AS = per_token_group_quant_int8(
        A_F.contiguous(), BLOCK_K, eps=EPS, dtype=torch.int8
    )

    def _block_quant_b(bf):
        """Per (block_n, block_k) absmax int8 quant of B (truncating cast)."""
        n, k = bf.shape
        nb = n // BLOCK_N
        kb = k // BLOCK_K
        codes = torch.empty(n, k, dtype=torch.int8, device=bf.device)
        ...
        for i in range(nb):
            for j in range(kb):
                ...
                q = torch.clamp(blk / scale, INT8_MIN, INT8_MAX).to(torch.int8)
...
def main():
    ...
    try:
        ...
        if status == "SUCCESS":
            lines.append("Inputs:\n")
            lines.append(f"- A(int8) shape: {stats['A_shape']}\n")
            lines.append(f"- B(int8) shape: {stats['B_shape']}\n")
...
def _run_with_crash_guard():
    ...
    if os.environ.get("W8A8_INT8_MM_CHILD") == "1":
        sys.exit(0 if main() == "SUCCESS" else 1)

    env = dict(os.environ, W8A8_INT8_MM_CHILD="1")
